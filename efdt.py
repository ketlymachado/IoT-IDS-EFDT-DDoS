...
def test_run():
    ...
    cols = ['pkts', 'bytes', 'ltime', 'seq', 'dur', 'mean', 'stddev', 'sum', 'min', 'max', 'spkts', 'dpkts', 'sbytes', 'dbytes', 'rate', 'srate', 'drate']
    ...
    title = list(botiot.columns.values)
    features = title[:-1]
    rows = botiot.shape[0]
    
    # convert df to data examples
    training_size = int(rows * 0.75)
    set_size = 145000
    ...
    array = botiot.head(training_size).values
    ...
    test_size = rows - training_size
    
    # Only for testing the algorithm
    #test_size = 50000

    test_set = botiot.tail(test_size).values
    x_test = test_set[:, :-1]
    y_test = test_set[:, -1]
